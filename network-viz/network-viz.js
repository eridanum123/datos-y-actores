...
.controller('NetworkVizCtrl', ["$scope", "$routeParams", "$http", function($scope, $routeParams, $http) {
    ...
    var force,
        data = [];
//    create_graph("network-viz/data/" + $routeParams.country + ".csv"); //Linea original
    create_graph("network-viz/data/" + $routeParams.country + ".csv", "", ""); //Mejora Ing Santiago Rojas


//================ CODIGO AÑADIDO POR SANTIAGO
    $scope.changeFuente = function() {
        create_graph("network-viz/data/" + $routeParams.country + ".csv", $scope.data.selectedFuente, $scope.data.selectedEnLinea);
    };
    $scope.changeEnLinea = function() {
        create_graph("network-viz/data/" + $routeParams.country + ".csv", $scope.data.selectedFuente, $scope.data.selectedEnLinea);
    };
    $scope.limpiarFiltrosCargarGrafoInicial = function() {
        $scope.data.selectedFuente = null;
        $scope.data.selectedEnLinea = null;
        create_graph("network-viz/data/" + $routeParams.country + ".csv", "", ""); //Mejora Ing Santiago Rojas
    }
// ================


//    function create_graph(filename) {  //Linea original
    function create_graph(filename, fuente, enLinea) {  //Mejora Ing Santiago Rojas
...
//================ CODIGO AÑADIDO POR SANTIAGO
                    ...
                    if((fuente != null && fuente != "") && (enLinea != null && enLinea != "")) {
                        data = dataTemp
                            .filter(function (d) {
                                return d['TIPO DE FUENTE'].toLowerCase() == fuente.toLowerCase() &&
                                    d['DISPONIBLE EN LÍNEA'].toLowerCase() == enLinea.toLowerCase();
                            })
                    }else if(fuente != null && fuente != ""){
                        data = dataTemp
                            .filter(function (d) {
                                return d['TIPO DE FUENTE'].toLowerCase() == fuente.toLowerCase();
                            })
                    }else if(enLinea != null && enLinea != ""){
                        data = dataTemp
                            .filter(function (d) {
                                return d['DISPONIBLE EN LÍNEA'].toLowerCase() == enLinea.toLowerCase();
                            })
                    }
// ================
